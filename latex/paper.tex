...
\begin{abstract}
Distributed AI systems require mechanisms for self-monitoring and adaptation. This paper specifies a Cognitive Resilience Layer (CRL) as a metacognitive control plane implementing an observe--evaluate--modify cycle over agent health and task-DAG state. In the current artifact, a baseline 10-agent, 20-task run completed all tasks with and without intervention, so no improvement is claimed for that benign condition. A separate stressed comparison with injected failures is included in the code and must be reported only after a fixed-seed repeated experiment. The contribution is therefore the formal control-plane design and reproducible evaluation protocol, not an unsupported resilience percentage.
\end{abstract}
...
\item A reproducible fault-injection evaluation protocol
\item Explicit separation of benign and stressed operating conditions
\end{itemize}
...
Failure probability (baseline) & 5\% \\
...
In the baseline configuration, both configurations achieve 100\% completion with zero standard deviation across the ten trials. This ceiling effect means that the benign experiment cannot measure resilience benefit. The repository therefore includes a stressed mode with earlier failure injection and must be used for the primary resilience claim.

\subsection{Anomaly Detection}

The single baseline run produced an average anomaly score of 0.02. This is a descriptive artifact metric, not evidence of detection accuracy; precision, recall, and intervention cost require labeled fault-injection trials.

\subsection{Limitations and Threats to Validity}

The agent and DAG models are synthetic, failures are injected randomly, and the current simulator does not model network partitions, recovery deadlines, or external workloads. Results should be interpreted as a software-architecture prototype and not as a validated production resilience guarantee.
